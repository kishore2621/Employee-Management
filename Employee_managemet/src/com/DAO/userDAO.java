...
public class userDAO 
{
	...
	public boolean add(Employee emp) {
		boolean f = false;
		try {
			String query = "insert into employee.emp_details values(?,?,?,?)";
	...
	public List<Employee> fetch() {
		...
		try {
			String query = "Select * from employee.emp_details";
			PreparedStatement preparedStatement = connection.prepareStatement(query);
	...
	public boolean update( Employee employee) {
		boolean f = false;
		try {
			String query = "update employee.emp_details set name = ?, email = ?, address = ? where id = ?";
	...
	public Employee getDataById(int id) {
		Employee employee = null;
		try {
			String query = "select * from employee.emp_details where id = ?";
			PreparedStatement preparedStatement = connection.prepareStatement(query);
	...
	public boolean delete(int id) {
		boolean f = false;
		try {
			String query = "Delete from employee.emp_details where id = ?";
			PreparedStatement preparedStatement = connection.prepareStatement(query);
			preparedStatement.setInt(1, id);
			int i = preparedStatement.executeUpdate();
			if(i == 1) {
